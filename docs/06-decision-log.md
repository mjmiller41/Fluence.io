...
- **Context:** Glowforge's cloud-lock is a documented, persistent user grievance; offline is LaserKerf's edge.
...
- **Consequence:** Also keeps the *server* footprint light (heavy compute is client-side).
...
## D-010 — Name: LaserKerf (laserkerf.io)
- **Context:** Working codename during planning was "WebBurn."
- **Decision:** Renamed to **LaserKerf**; project file format `.laserkerf`; repo `git@github.com:mjmiller41/LaserKerf.io.git`.
...
- Decide telemetry (external SaaS assumed) and whether cloud project-sync is in scope.
